 ...
 return {row = draw.row, col = draw.col, z = (10 + card)} elseif ((_G.type(location) == "table") and (location[1] == "complete") and (nil ~= location[2]) and (nil ~= location[3])) then local n = location[2] local card = location[3]


 return {row = draw.row, col = (-5 + tableau.col + (card_col_step * 4) + (n * card_col_step)), z = ((n * 10) + card)} else local _ = location
 ...
 M.start = function(app_config, game_config, _3fseed) _G.assert((nil ~= game_config), "Missing argument game-config on fnl/playtime/game/curds-and-whey/app.fnl:44") _G.assert((nil ~= app_config), "Missing argument app-config on fnl/playtime/game/curds-and-whey/app.fnl:44")
 return PatienceApp.start({name = "Curds & Whey", filetype = "curds-and-whey", view = {width = 119, height = 42}, ["empty-fields"] = {{"tableau", 13}, {"complete", 4}}, ["card-style"] = {colors = 4}}, {AppImpl = M, LogicImpl = Logic, StateImpl = AppState}, app_config, game_config, _3fseed) end
