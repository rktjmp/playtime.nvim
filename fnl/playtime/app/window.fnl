...
;(fn M.render-classic [view component-layers]
;  ;;
...
;        component-positions {}
;        memoise (fn memoise [component]
;                  (when component.visible?
;                    (let [pos (.. component.row "." component.col)]
;                      (if (not (. component-positions pos))
;                        (tset component-positions pos []))
;                      (table.insert (. component-positions pos) component))
;                    (each [_ child (ipairs (or component.children []))]
;                      (memoise child))))
;        _ (each [_ layer (ipairs component-layers)]
;            (each [_ component (ipairs layer)]
;              (memoise component)))]
;    (fn draw [component row col]
;      (when (and component.visible? (= row component.row) (= col component.col))
;        (let [{: width : height :tag ?tag : z} component]
;          (for [line 1 height]
...
;        (when (and component.visible? component.children)
;          (each [_ child (ipairs component.children)]
;            (draw child row col))))
;    (api.nvim_buf_clear_namespace buf ns 0 -1)
;    (for [row 0 (- overflow-height 1)]
;      (for [col 0 (- frame-width 1)]
;        (case (. component-positions (.. row "." col))
;          components (each [_ c (ipairs components)]
;                       (draw c row col)))))
